...
export default function StaffPage() {
  ...
  const { data, isLoading, refetch, isFetching } = useQuery({
    ...
    queryFn: () => api<{ data: AdminUser[]; total: number }>(`/v1/admin/users?search=${encodeURIComponent(search)}&limit=100`),
    staleTime: 30_000,
  });
    ...
    <AppShell>
      ...
      {!isLoading && users.length === 0 && (
        <div className="flex flex-col items-center justify-center gap-4 py-20 text-center">
          <div className="flex size-14 items-center justify-center rounded-2xl border bg-muted"><IconUsers className="size-7 text-muted-foreground" /></div>
          <div className="space-y-1"><h2 className="text-lg font-semibold">No users found</h2><p className="max-w-xs text-sm text-muted-foreground">Try adjusting your search.</p></div>
        </div>
      )}
              ...
              {users.map((u) => {
                ...
                return (
                  <TableRow key={u._id} className="h-12">
                    ...
                    <TableCell>
                      <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize ${ROLE_STYLE[role] ?? "bg-muted text-muted-foreground"}`}>
                        {role.replace(/_/g, " ")}
                      </span>
                    </TableCell>
                    <TableCell>
                      <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${isActive ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400" : "bg-muted text-muted-foreground"}`}>
